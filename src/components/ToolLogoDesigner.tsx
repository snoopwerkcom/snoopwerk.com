...
const ToolLogoDesigner: React.FC<ToolLogoDesignerProps> = ({ state, credits, onUpdate, onUpdateCredits, onAction }) => {
 ...
 const handleGenerate = async () => {
  ...
  if (state.view === 'LANDING') {
    return (
      <div className="h-full relative overflow-hidden bg-[#020202] flex flex-col font-inter">
        {/* Cinematic Backdrop with Transparent Logo Assets on Alpha Grid */}
        <div className="absolute inset-0 z-0 flex group/bg">
          {/* Transparency Grid Pattern Base */}
          <div 
          ...
          {/* Grid of Minimalist & Transparent Logo Concepts - Opacity set to 100% */}
  ...
  // Production View (The Laboratory)
  return (
    <div className="h-full flex flex-col lg:flex-row bg-[#050505] overflow-hidden relative font-inter">
          ...
          <div className="flex gap-2">
            {state.image && (
              <button 
                ...
                className="px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest bg-blue-600 text-white hover:bg-blue-500 transition-all shadow-lg active:scale-95"
              >
                Export Asset
              </button>
            )}
            <button 
              ...
              className="px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest bg-white/5 border border-white/10 text-slate-400 hover:text-white transition-all hover:bg-white/10 disabled:opacity-30"
            >
              UPSCALE
            </button>
          ...
          <div className="relative w-full h-full max-w-2xl bg-[#0a0a0a] rounded-[56px] border border-white/5 shadow-[0_48px_96px_-12px_rgba(0,0,0,0.9)] overflow-hidden flex items-center justify-center group">
            ...
            {state.image ? (
              ...
              <div 
                ...
                className="text-center opacity-20 cursor-pointer hover:opacity-40 transition-all z-10 flex flex-col items-center gap-6"
              >
      ...
      <aside className="w-full lg:w-96 border-l border-white/5 bg-[#080808] flex flex-col shrink-0 shadow-2xl relative z-30 overflow-hidden">
        ...
        <div className="p-8 flex-1 overflow-y-auto custom-scrollbar space-y-12 pb-24">
          ...
          {state.image && (
            <div className="animate-in slide-in-from-right duration-500 space-y-12 pt-12 border-t border-white/5">
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] ml-1">Brandmark Text</label>
                <input 
                  type="text" 
                  value={state.edit.overlayText}
                  onChange={(e) => updateEdit({ overlayText: e.target.value })}
                  className="w-full bg-[#121212] border border-white/10 rounded-[20px] p-5 text-sm text-white focus:ring-1 focus:ring-blue-600 outline-none shadow-inner"
              ...
              <div className="space-y-4">
                ...
                <textarea 
                  ...
                  className="w-full bg-[#121212] border border-white/10 rounded-[28px] p-6 text-sm text-white h-32 focus:ring-1 focus:ring-blue-600 outline-none resize-none shadow-inner"
                  placeholder="Custom refinement instructions..."
                />
                <button 
                  onClick={handleApplyMagicEdit}
                  disabled={magicLoading}
                  className="w-full py-6 bg-blue-600 text-white font-black rounded-3xl text-[11px] uppercase tracking-[0.4em] hover:bg-blue-700 transition-all shadow-2xl active:scale-95 disabled:opacity-50"
                >
                  APPLY REFINEMENTS
                </button>
              </div>
